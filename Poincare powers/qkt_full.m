...
(* ::Input:: *)
(*plot=ListDensityPlot[Transpose[{region[[All,1]],region[[All,2]],newdata}],PlotRange->All,PlotTheme->"Detailed",ColorFunction->"SunsetColors",PlotLabel->Style["D\:2082, J = "<>ToString[J]<>", \[Alpha] = "<>ToString[N[\[Alpha]]]<>", k = "<>ToString[NumberForm[k,{4,2}]]<>" (rotation-kick)",25,Black],ImageSize->600,FrameLabel->{Style["Q",40,Black],Rotate[Style["P",40,Black],-90 Degree]},LabelStyle->Directive[30,Black],AspectRatio->1,Frame->True,Background->White]*)
...
(*plot=ListDensityPlot[husimiList,PlotRange->All,PlotTheme->"Detailed",ColorFunction->"BlueGreenYellow",PlotLabel->Style["AE Husimi, J = "<>ToString[J]<>", \[Alpha] = "<>ToString[N[\[Alpha]]]<>", k = "<>ToString[NumberForm[k,{4,2}]]<>", Eigenvec_q = "<>ToString[q],25,Black],ImageSize->1000,FrameLabel->{Style["Q",40,Black],Rotate[Style["P",40,Black],-90 Degree]},LabelStyle->Directive[30,Black],AspectRatio->1,Frame->True,InterpolationOrder->1];*)
...
(*result=ClassicalMapeo[randomregion,\[Alpha],k,kicks,0];*)


(* ::Input:: *)
(*newpoincarea=ArrayReshape[result,{points*kicks,2}];*)
(*poincare=ListPlot[newpoincarea,PlotStyle->Directive[PointSize[0.0005],Black,Opacity[0.15]],PlotLabel->Row[{Style["\[Alpha] = ",35,Black],Style[N[\[Alpha]],35,Black],Style[", k = ",35,Black], Style[N[k],35,Black],Style[" (rotation-kick)",24,Black]}],FrameLabel->{Style["Q",30,Black],Rotate[Style["P",30,Black],-90Degree]},LabelStyle->Directive[ 30,Black],AspectRatio->1,Method->{"AxesInFront"->True},Method->{"GridInFront"->True},Frame->True,Axes->True,GridLines->Automatic,PlotRange->{{-2,2},{-2,2}},ImageSize->1000]*)
