...
sz[j_] := DiagonalMatrix[Table[-j + k - 1, {k, 1, 2 j + 1}]];
sp[j_] := DiagonalMatrix[Table[Sqrt[j (j + 1) - m (m + 1)], {m, -j, j - 1}], 1];
sm[j_] := DiagonalMatrix[Table[Sqrt[j (j + 1) - m (m - 1)], {m, -j + 1, j}], -1];
sx[j_] := SparseArray[(1/2) (sp[j] + sm[j])];
sy[j_] := SparseArray[(1/(2 I)) (sp[j] - sm[j])];
sx2[j_] := sx[j] . sx[j];
...
Floqsym[j_, \[Alpha]_, k_] := twistPart[j, \[Alpha]/2] . kickPart[j, k] . twistPart[j, \[Alpha]/2];
